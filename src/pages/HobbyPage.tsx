...
export function HobbyPage({ hobby }: { hobby: Hobby }) {
  const hasHobbyUrl = hobby.hobbyUrl && hobby.hobbyUrl.trim().length > 0;
  const paragraphs: string[] = Array.isArray(hobby.longDescription)
    ? hobby.longDescription
    : (hobby.longDescription || hobby.shortDescription || hobby.description || '')
        .split(/\n\n+/)
        .filter(Boolean);

  const backHref = '/#hobbies';
  const backLabel = '← Back to Hobbies';
    ...
    <main id="main-content" className="site-main flex-1" tabIndex={-1}>
      <section className="max-w-4xl mx-auto content-card shadow-lg p-6 sm:p-10 space-y-8">
        <div className="space-y-4">
          <h2 className="text-3xl md:text-4xl font-extrabold tracking-tight text-primary">{hobby.title}</h2>
        ...
        <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-base-content/10">
          {hasHobbyUrl && (
            <a href={hobby.hobbyUrl} className="btn btn-primary rounded-full px-6" target="_blank" rel="noreferrer">
              Learn More ↗
            </a>
          )}
